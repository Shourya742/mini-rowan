...
use crate::{
    pure::{PureChild, PureChildKind, PureToken, PureTree},
    sll::{Elem, adjust, link, unlink},
};
...
impl SyntaxTree {
    ...
    fn get_child(&self, index: usize) -> Option<SyntaxChild> {
        ...
        link(&self.data.first, res.data_mut());
        Some(res)
    }
            ...
            match node.parent() {
                Some(parent) => {
                    pure = parent
                        .pure()
                        .borrow()
                        .replace_child(node.data.index.get(), pure.into());
                    node = parent
                }
                None => return,
            }
...
impl From<PureTree> for SyntaxTree {
    fn from(value: PureTree) -> Self {
        SyntaxTree::new(value)
    }
}
...
impl SyntaxData {
    ...
    fn detach(self: &Rc<SyntaxData>) {
        ...
        adjust(&self, self.index.get() + 1, -1);
        self.unlink()
    }
