...
impl GreenNode {
        ...
        I::IntoIter: ExactSizeIterator,
    {
        ...
        // text_len after construction because we can't iterate `slots` twice.
        let data = {
    ...
    pub(crate) fn into_raw(self) -> ptr::NonNull<GreenNodeData> {
        ...
        // is safe since `GreenNodeData` is marked as `repr(transparent)`
...
impl<'a> TryFrom<(usize, &'a Slot)> for Child<'a> {
    ...
    fn try_from((index, slot): (usize, &'a Slot)) -> Result<Self, Self::Error> {
        match slot {
            Slot::Empty { .. } => Err(()),
            Slot::Node { rel_offset, node } => Ok(Child {
                slot: index as u32,
                rel_offset: *rel_offset,
                element: NodeOrToken::Node(node),
            }),
            Slot::Token { rel_offset, token } => Ok(Child {
                slot: index as u32,
                rel_offset: *rel_offset,
