...
use crate::{
    syntax::SyntaxKind,
    syntax_factory::{
        parsed_children::{ParsedChildren, ParsedChildrenIntoIterator},
        raw_syntax::{RawSyntaxElement, RawSyntaxNode},
    },
};
pub mod parsed_children;
pub mod raw_syntax;

/// Factory for creating syntax nodes of a particular kind.
pub trait SyntaxFactory: Debug {
    ...
    /// Creates a *node list* syntax node. Validates if all elements are valid and changes the node's kind to
